...
import xyz.maywr.hack.api.mixin.mixins.network.AccessorCPacketUseEntity;
import xyz.maywr.hack.api.property.Setting;
import xyz.maywr.hack.api.util.Timer;
import xyz.maywr.hack.api.util.*;
...
public final class AutoCrystal extends Module {
    ...
    //render page
    private final Setting<Integer> red = register(new Setting<>("Red", 255, 0, 255, v -> page.getValue() == Page.RENDER));
    private final Setting<Integer> green = register(new Setting<>("Green", 255, 0, 255, v -> page.getValue() == Page.RENDER));
    private final Setting<Integer> blue = register(new Setting<>("Blue", 255, 0, 255, v -> page.getValue() == Page.RENDER));
    ...
    private final xyz.maywr.hack.api.util.Timer clearTimer = new xyz.maywr.hack.api.util.Timer();
    private final xyz.maywr.hack.api.util.Timer breakTimer = new Timer();
    ...
    public void onTick() {
        ...
        if (!offhand && mc.player.getHeldItemMainhand().getItem() != Items.END_CRYSTAL && !autoSwitch.getValue())
            return;
        if (!MaywrWare.eventManager.switchTimerPassed(cooldown.getValue())) {
            return;
        }
    ...
    public void onRender3D() {
        if (renderPos != null) {
            RenderUtil.drawBoxESP(renderPos , new Color(red.getValue(), green.getValue(), blue.getValue()), 1, true, true, alpha.getValue(), 1);
        }
    }
