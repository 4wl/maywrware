...
import xyz.maywr.hack.api.mixin.mixins.render.AccessorRenderManager;
import xyz.maywr.hack.api.property.Setting;
import xyz.maywr.hack.api.util.EntityUtil;
...
public final class Nametags extends Module {
    ...
    private void renderNameTag(EntityPlayer player, double x, double y, double z, float delta) {
        ...
        if(rect.getValue()) {
            if (gradientBorder.getValue()) {
                ...
                RenderUtil.drawBorderedRect(-width - 2, -(mc.fontRenderer.FONT_HEIGHT + 1), width + 2F, 1.5F, 0x55000000, rectBorder.getValue() ? (MaywrWare.friendManager.isFriend(player) ? 0xFF55C0ED : Color.WHITE.getRGB()) : 0x00000000);
            }
        }
    ...
    private String getDisplayTag(EntityPlayer player) {
        ...
        return name + " " + ChatFormatting.RESET + pingStr;
    }

    private int getDisplayColour(EntityPlayer player) {
        if (MaywrWare.friendManager.isFriend(player)) {
            return 0xFF55C0ED;
