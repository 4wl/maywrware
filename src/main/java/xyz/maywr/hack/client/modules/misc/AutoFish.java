...
@ModuleManifest(name = "AutoFish", category = Module.Category.MISC, description = "getting fish for you")
public class AutoFish extends Module {
    ...
    public void onEnable() {
        if (webhook == null && mode.getValue().equals(Mode.ADVANCED)) {
            notify = false;
            MessageUtil.sendClientMessage("please enter a discord webhook using " + ClickGui.getInstance().prefix.getValue() + "webhook <url>", false);
            this.disable();
        }
    }
    ...
    public void onFishedItem(ItemFishedEvent event) {
        ...
        try {
            sendFishEmbed(
                    MaywrWare.modid + " " + MaywrWare.VERSION,
                    item.getDisplayName(),
    ...
    private static void sendFishEmbed(String author, String itemName,
        ...
        try {
            ...
            MessageUtil.sendClientMessage("can't send item to webhook, check da console for error", false);
            e.printStackTrace();
        }
    }
